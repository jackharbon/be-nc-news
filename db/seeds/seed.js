...
const seed = async ({ topicData, userData, articleData, commentData }) => {
  ...
  CREATE TABLE comments (
	...
	const topicsPromise = db.query(insertTopicsQueryStr).then((result) => result.rows);
	...
	const insertArticlesQueryStr = format(
		'INSERT INTO articles (title, topic, author, body, img_url, created_at, votes) VALUES %L RETURNING *;',
		formattedArticleData.map(({ title, topic, author, body, img_url, created_at, votes = 0 }) => [
			title,
			topic,
			author,
			body,
	...
	const articleRows = await db.query(insertArticlesQueryStr).then((result) => result.rows);
